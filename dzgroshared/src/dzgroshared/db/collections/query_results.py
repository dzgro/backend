...
from dzgroshared.models.collections.analytics import ComparisonPeriodDataRequest
from dzgroshared.db.DbUtils import DbManager
from dzgroshared.db.DataTransformer import Datatransformer
from dzgroshared.models.collections.query_results import PerformanceTableRequest
...
class QueryResultsHelper:
    ...
    def __init__(self, client: DzgroSharedClient) -> None:
        self.client = client
        self.db = DbManager(client.db.database.get_collection(CollectionType.QUERY_RESULTS), marketplace=client.marketplaceId)
    ...
    async def getPerformanceListforPeriod(self, body: PerformanceTableRequest):
        ...
        pipeline: list[dict] = [self.db.pp.matchMarketplace(matchDict), Analytics.getProjectionStage('Comparison', body.collatetype), sort, skip, limit]
        ...
        data = Analytics.transformData('Comparison',data, body.collatetype, self.client.marketplace.countrycode)
        ...
        columns.extend([item.metric.value for item in Analytics.getMetricGroupsBySchemaType('Comparison', body.collatetype)])
    ...
    async def getPerformanceforPeriod(self, req: ComparisonPeriodDataRequest):
        ...
        data = await self.db.aggregate(pipeline)
        data = Analytics.transformData('Comparison',data, req.collatetype, self.client.marketplace.countrycode)
        return data[0]
