...
class QueryHelper:
    ...
    async def getQueryTable(self, req: SingleMetricPeriodDataRequest):
        lookupLet = { 'uid': self.uid, 'marketplace': self.marketplace, 'queryid': '$_id', 'collatetype': 'marketplace' }
        if req.value: lookupLet['value'] = req.value
        lookupMatch = { '$expr': { '$and': [ { '$eq': [ '$uid', '$$uid' ] }, { '$eq': [ '$marketplace', '$$marketplace' ] }, { '$eq': [ '$queryid', '$$queryid' ] }, { '$eq': [ '$collatetype', '$$collatetype' ] } ] } }
        ...
        data = Analytics.transformCurrPreData(data, req.countrycode)
        return data
