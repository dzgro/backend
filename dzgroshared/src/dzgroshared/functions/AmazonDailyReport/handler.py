...
from dzgroshared.amazonapi import AmazonApiClient, AmazonApiObject
...
class AmazonReportManager:
    ...
    def __init__(self, fnclient: FunctionClient):
    ...
    async def checkMessage(self):
        try:
            ...
            if self.report: await self.fnclient.client.db.amazon_daily_reports.terminateParent(self.report.id, error)
    
    async def setUserMarketplace(self):
        self.userMarketplace = MarketplaceObjectForReport(**await self.fnclient.client.db.marketplaces.getMarketplaceObjectForReport(ObjectId(self.message.marketplace)))
        self.reportUtil = ReportUtil(self.fnclient, self.userMarketplace)
    ...
    async def executeMessage(self)->tuple[AmazonParentReportQueueMessage, str]|None:
        if not self.message.index:
            ...
            self.report = await self.fnclient.client.db.amazon_daily_reports.getParentReport(reportid)
            if self.message.step==AmazonDailyReportAggregationStep.ADD_PRODUCTS:
                ...
                builder = ListingsBuilder(self.fnclient,  self.userMarketplace, await self.spapi(), self.reportUtil, self.report.id)
                ...
                if self.message.date: return await self.__sendMessage(AmazonDailyReportAggregationStep.ADD_PRODUCTS)
                    ...
                    await self.fnclient.client.db.amazon_daily_reports.markProductsComplete(reportid)
                ...
                if self.report.progress!=100: 
                    ...
                    await self.fnclient.client.db.amazon_daily_reports.markReportsComplete(reportid)
                ...
                if not self.report.productsComplete or self.report.progress!=100:
                    ...
                    await self.reportUtil.update(self.fnclient.client.db, CollectionType.ADV_ASSETS, portfolios, self.report.id)
                ...
                for assetType in [AdAssetType.AD_GROUP, AdAssetType.CAMPAIGN, AdAssetType.PORTFOLIO, AdAssetType.AD]:
                    ...
                    await self.fnclient.client.db.adv_assets.db.aggregate(pipeline)
                ...
                processor = AnalyticsProcessor(self.fnclient.client.db, self.userMarketplace,self.report)
                ...
                builder = QueryBuilder(self.fnclient.client.db, self.userMarketplace, self.report)
                ...
                await self.fnclient.client.db.amazon_daily_reports.deleteChildReports(self.message.index)
                await self.fnclient.client.db.amazon_daily_reports.markParentAsCompleted(self.message.index)
                if self.report.createdat:
                    await self.fnclient.client.db.marketplaces.completeReportProcessing(
    ...
    async def __sendMessage(self, step: AmazonDailyReportAggregationStep, delay: int=0):
        ...
        messageId = (await self.fnclient.client.sqs.mockMessage(SendMessageRequest(QueueUrl=QueueUrl.AMAZON_REPORTS, DelaySeconds=delay), self.message)).message_id
        return self.message, messageId

    async def spapi(self):
        if self.spapiClient is None:
            obj = AmazonApiObject(**await self.fnclient.client.db.marketplaces.getMarketplaceApiObject(
                self.userMarketplace.id,self.fnclient.client.secrets.SPAPI_CLIENT_ID,
                self.fnclient.client.secrets.SPAPI_CLIENT_SECRET, AmazonAccountType.SPAPI))
            self.spapiClient = self.fnclient.client.amazonapi(obj).spapi
        return self.spapiClient

    async def adapi(self):
        if self.adapiClient is None:
            obj = AmazonApiObject(**await self.fnclient.client.db.marketplaces.getMarketplaceApiObject(
                self.userMarketplace.id,self.fnclient.client.secrets.ADS_CLIENT_ID,
                self.fnclient.client.secrets.ADS_CLIENT_SECRET, AmazonAccountType.ADVERTISING))
            self.adapiClient = self.fnclient.client.amazonapi(obj).ad
        return self.adapiClient

    async def getSPAPIReportManager(self) -> AmazonSpapiReportManager:
        if self.spapiReportManager is None:
            self.spapiReportManager = AmazonSpapiReportManager(self.fnclient, self.userMarketplace, await self.spapi())
    ...
    async def getDataKioskReportManager(self) -> AmazonDataKioskReportManager:
        if self.dataKioskReportManager is None:
            self.dataKioskReportManager = AmazonDataKioskReportManager(self.fnclient, self.userMarketplace, await self.spapi())
    ...
    async def getAdReportManager(self) -> AmazonAdsReportManager:
        if self.adReportManager is None:
            self.adReportManager = AmazonAdsReportManager(self.fnclient, self.userMarketplace, await self.adapi())
        return self.adReportManager
    
    async def getAdExportManager(self) -> AmazonAdsExportManager:
        if self.adExportManager is None:
            self.adExportManager = AmazonAdsExportManager(self.fnclient, self.userMarketplace, await self.adapi())
    ...
    async def createReports(self):
        ...
        self.message.index = str(await self.fnclient.client.db.amazon_daily_reports.insertParentReport(startDate, endDate, reports))
