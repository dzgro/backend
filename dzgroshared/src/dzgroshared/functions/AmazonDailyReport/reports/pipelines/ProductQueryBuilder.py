...
from dzgroshared.models.model import QueryBuilderValue
...
class QueryBuilder:
    ...
    def __init__(self, dbClient: DbClient, startdate: datetime, enddate: datetime):
    ...
    def getQuery(self, query: QueryBuilderValue)->Query|None:
        if query.tag==CollateTypeTag.DAYS_7:
            return Query(_id=query.id, tag=query.tag, 
                  curr=QueryPeriod(start=self.enddate-timedelta(days=6), end=self.enddate, label=''),
                  pre=QueryPeriod(start=self.enddate-timedelta(days=13), end=self.enddate-timedelta(days=7), label='')
                  )
        elif query.tag==CollateTypeTag.DAYS_30:
            return Query(_id=query.id, tag=query.tag,
                  curr=QueryPeriod(start=self.enddate-timedelta(days=29), end=self.enddate, label=''),
                  pre=QueryPeriod(start=self.enddate-timedelta(days=59), end=self.enddate-timedelta(days=30), label='')
                  )
        elif query.tag==CollateTypeTag.MONTH_ON_NONTH:
            currdates = self.get_month_datetimes_till(self.enddate)
            preDates = self.get_prev_month_datetimes_till_same_day(self.enddate)
            return Query(_id=query.id, tag=query.tag,
            ...
            currdates = self.get_month_datetimes_till(self.enddate)
            preDates = self.get_all_dates_of_prev_month(self.enddate)
            return Query(_id=query.id, tag=query.tag,
                  curr=QueryPeriod(start=currdates[0], end=currdates[-1], label=''),
                  pre=QueryPeriod(start=preDates[0], end=preDates[-1], label=''),
                  )
    ...
    async def execute(self, query: Query):
        ...
        lookupAnalytics = { '$lookup': { 'from': 'date_analytics', 'let': { 'uid': self.dbClient.uid, 'marketplace': self.dbClient.marketplace, 'dates': { '$setUnion': [ '$curr', '$pre' ] } }, 'pipeline': [ { '$match': { '$expr': { '$and': [ { '$eq': [ '$uid', '$$uid' ] }, { '$eq': [ '$marketplace', '$$marketplace' ] }, { '$in': [ '$date', '$$dates' ] } ] } } }, { "$project": { "_id": 0 } } ], 'as': 'data' } }
