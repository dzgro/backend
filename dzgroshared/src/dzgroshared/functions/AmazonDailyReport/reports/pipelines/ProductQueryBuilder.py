...
from dzgroshared.models.model import QueryBuilderValue, StartEndDate
...
class QueryBuilder:
    ...
    async def getTags(self):
        data = await self.client.db.queries.db.find({"marketplace": {"$exists": False}})
    ...
    def getQuery(self, query: QueryBuilderValue)->Query|None:
        if query.tag==CollateTypeTag.DAYS_7:
            return Query(_id=query.id, tag=query.tag, 
                  curr=QueryPeriod(start=self.dates.enddate-timedelta(days=6), end=self.dates.enddate, label=''),
                  pre=QueryPeriod(start=self.dates.enddate-timedelta(days=13), end=self.dates.enddate-timedelta(days=7), label='')
                  )
        elif query.tag==CollateTypeTag.DAYS_30:
            return Query(_id=query.id, tag=query.tag,
                  curr=QueryPeriod(start=self.dates.enddate-timedelta(days=29), end=self.dates.enddate, label=''),
                  pre=QueryPeriod(start=self.dates.enddate-timedelta(days=59), end=self.dates.enddate-timedelta(days=30), label='')
                  )
        elif query.tag==CollateTypeTag.MONTH_ON_NONTH:
            currdates = self.get_month_datetimes_till(self.dates.enddate)
            preDates = self.get_prev_month_datetimes_till_same_day(self.dates.enddate)
            return Query(_id=query.id, tag=query.tag,
            ...
            currdates = self.get_month_datetimes_till(self.dates.enddate)
            preDates = self.get_all_dates_of_prev_month(self.dates.enddate)
            return Query(_id=query.id, tag=query.tag,
                  curr=QueryPeriod(start=currdates[0], end=currdates[-1], label=''),
                  pre=QueryPeriod(start=preDates[0], end=preDates[-1], label=''),
                  )
    ...
    async def getNextQuery(self, query: Query|None):
        if not query: await self.client.db.query_results.db.deleteMany({})
    ...
    async def execute(self, query: Query):
        ...
        lookupAnalytics = { '$lookup': { 'from': 'date_analytics', 'let': { 'uid': self.client.db.uid, 'marketplace': self.client.db.marketplace, 'dates': { '$setUnion': [ '$curr', '$pre' ] } }, 'pipeline': [ { '$match': { '$expr': { '$and': [ { '$eq': [ '$uid', '$$uid' ] }, { '$eq': [ '$marketplace', '$$marketplace' ] }, { '$in': [ '$date', '$$dates' ] } ] } } }, { "$project": { "_id": 0 } } ], 'as': 'data' } }
        ...
        dt = Datatransformer(self.client.db.queries.db.pp)
        ...
        pipeline.extend([{ '$set': { 'uid': self.client.db.uid, 'marketplace': self.client.db.marketplace, 'queryid': '$_id', '_id': { '$concat': [ { '$toString': '$marketplace' }, '_', { '$toString': '$_id' }, '_', '$collatetype', '_', { '$toString': '$value' } ] }, 'parent': { '$switch': { 'branches': [ { 'case': { '$eq': [ '$collatetype', 'sku' ] }, 'then': '$asin' }, { 'case': { '$eq': [ '$collatetype', 'asin' ] }, 'then': '$parentsku' }, { 'case': { '$eq': [ '$collatetype', 'parentsku' ] }, 'then': '$producttype' } ], 'default': None } } } }, { '$project': { 'data': 1, 'queryid': 1, 'value': 1, 'collatetype': 1, 'parent': 1 } }])
        pipeline.append(self.client.db.queries.db.pp.merge(CollectionType.QUERY_RESULTS, 'replace', 'insert'))
        await self.client.db.queries.db.aggregate(pipeline)
