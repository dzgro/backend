def build_layer_zip_clean():
	"""
	Creates dzgroshared_layer.zip at the project root and deletes intermediary build files after zipping.
	"""
	import os
	import shutil
	import zipfile
	...
	shared_src = os.path.join(project_root, 'dzgroshared', 'src', 'dzgroshared')
	layer_dir = os.path.join(os.path.dirname(__file__), 'layer_build', 'python', 'lib', 'python3.12', 'site-packages')
	layer_zip = os.path.join(project_root, 'dzgroshared_layer.zip')

	# Ensure layer build directory exists
	os.makedirs(layer_dir, exist_ok=True)

	# Remove previous build dir if exists
	if os.path.exists(layer_dir):
		shutil.rmtree(layer_dir)
	shutil.copytree(shared_src, os.path.join(layer_dir, 'dzgroshared'))

	# Zip the layer
	with zipfile.ZipFile(layer_zip, 'w', zipfile.ZIP_DEFLATED) as zf:
		for root, dirs, files in os.walk(os.path.join(layer_dir, 'dzgroshared')):
			for file in files:
	...
	# Delete intermediary build files
	if os.path.exists(layer_dir):
		shutil.rmtree(layer_dir)
	print(f"Deleted intermediary build directory: {layer_dir}")
