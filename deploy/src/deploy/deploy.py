...
def main():
    ...
    try:
        ...
        if env != ENVIRONMENT.LOCAL:
            docker_started = start_docker_desktop()
            if not docker_started:
                raise ValueError("Warning: Docker Desktop failed to start. Docker builds may fail.")
        ...
        # Close Docker Desktop if we started it
        if docker_started and env != ENVIRONMENT.LOCAL:
