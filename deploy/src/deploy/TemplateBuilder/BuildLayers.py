...
from deploy.TemplateBuilder.StarterMapping import LAYER_DEPENDENCIES, LAYER_NAME, Region, ENVIRONMENT
import os
import hashlib
import json
import time
...
class LambdaLayerBuilder:
    ...
    def __init__(self, env: ENVIRONMENT, region: Region, max_workers: int = 4):
        ...
        # Initialize caching directories - store in deploy directory
        deploy_dir = Path(__file__).parent.parent.parent.parent  # deploy/ directory
    ...
    def _load_dependency_hashes(self) -> Dict[str, str]:
        ...
        try:
            ...
            print(f"⚠️ Warning: Could not load dependency hashes: {e}")
        return {}
    ...
    def _save_dependency_hashes(self):
        ...
        try:
            ...
            print(f"⚠️ Warning: Could not save dependency hashes: {e}")
    ...
    def get_layer_name(self, layer_name: LAYER_NAME) -> str:
        ...
        return f"{layer_name.value.lower()}-deps-{self.envtextlower}"
    ...
    def get_custom_layer_name(self, layer_name: LAYER_NAME) -> str:
        ...
        if layer_name == LAYER_NAME.DZGRO_SHARED:
            return f"dzgroshared-{self.envtextlower}"
        elif layer_name == LAYER_NAME.API:
            return f"api-{self.envtextlower}"
        else:
            # Fallback to standard naming
            return f"{layer_name.value.lower()}-{self.envtextlower}"
...
'''
    ...
    '''
        ...
        try:
            ...
            if os.path.exists(dockerfile_path):
                os.remove(dockerfile_path)
                print("🧹 Cleaned up Dockerfile.layer")
    ...
    def cleanup_old_layer_versions(self, layer_name: str, keep_versions: int = 3):
        ...
        try:
            ...
            if len(versions) <= keep_versions:
                print(f"✅ Layer {layer_name} has {len(versions)} versions, no cleanup needed")
                return
            ...
            print(f"🧹 Cleaning up {len(versions_to_delete)} old versions of layer {layer_name}")
            print(f"📁 Keeping latest {keep_versions} versions: {[v['Version'] for v in versions[:keep_versions]]}")
            ...
            for version in versions_to_delete:
                try:
                    ...
                    print(f"   ⚠️  Failed to delete version {version['Version']}: {e}")
            
            print(f"✅ Successfully deleted {deleted_count} old layer versions")
            
        except Exception as e:
            print(f"⚠️  Warning: Failed to cleanup old layer versions for {layer_name}: {e}")
    ...
    def cleanup_cache(self):
        ...
        try:
            if self.cache_dir.exists():
                ...
                if zip_files:
                    print(f"✅ Cleaned up {len(zip_files)} layer files + metrics")
                    print(f"📁 Kept only dependency_hashes.json for cache validation")
                else:
                    print("📁 No layer files to clean up")
        except Exception as e:
            print(f"⚠️  Warning: Failed to cleanup cache: {e}")
