...
from models.collections.queries import Query, QueryPeriod
from models.enums import CollateTypeTag, CollectionType
from models.extras.amazon_daily_report import AmazonParentReport, MarketplaceObjectForReport, QueryBuilderValue
...
class QueryBuilder:
    ...
    def __init__(self, marketplace: MarketplaceObjectForReport, report: AmazonParentReport, dbClient: DbClient):
        ...
        self.report = report
        self.queryDb = dbClient.queries(self.marketplace.uid, ObjectId(self.marketplace.id))
    ...
    async def getTags(self):
        data = await self.queryDb.db.find({"marketplace": {"$exists": False}})
    ...
    async def execute(self, query: Query):
        ...
        dt = Datatransformer(self.queryDb.db.pp)
