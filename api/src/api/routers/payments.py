...
from models.collections.payments import Payments, Payment
...
async def getPayments(request: Request, paginator: Paginator):
    return await RequestHelper(request).payments.getPayments(paginator)
...
async def generateInvoiceLink(request: Request, id: str):
    _payments = RequestHelper(request).payments
    ...
    payment.invoicelink = S3Storage().create_signed_url_by_path(path)
    return payment
