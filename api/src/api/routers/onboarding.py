...
async def getOnboardingManager(request: Request)->OnboardingHelper:
    return OnboardingHelper((await RequestHelper(request).client))
...
async def addMarketplace(request: Request, req: AddMarketplace):
    return await (await getOnboardingManager(request)).addMarketplace(req)

@router.post("/test-linkage", response_model=SuccessResponse)
async def testLinkage(request: Request, req: AddMarketplace):
    return await (await getOnboardingManager(request)).testLinkage(req)
