...
class Datatransformer:
    ...
    def collateData(self):
        return self.pp.collateListOfObjectsAsObject(self.datakey)
    ...
    #     x = { '$concatArrays': [ '$$value', [ { 'k': '$$this.k', 'v': { '$let': { 'vars': { 'ispercent': { '$in': [ '$$this.k', '$percentkeys' ] } }, 'in': { '$arrayToObject': { '$reduce': { 'input': { '$objectToArray': '$$this.v' }, 'initialValue': [], 'in': { '$concatArrays': [ '$$value', [ { 'k': '$$this.k', 'v': { '$cond': { 'if': { '$in': [ '$$this.k', [ 'curr', 'pre' ] ] }, 'then': { '$let': { 'vars': { 'val': '$$this.v' }, 'in': { '$let': { 'vars': { 'calculatedVal': { '$ifNull': [ { '$reduce': { 'input': [ { 'val': 10000000, 'key': 'Cr' }, { 'val': 100000, 'key': 'Lacs' }, { 'val': 1000, 'key': 'K' } ], 'initialValue': None, 'in': { '$cond': [ { '$and': [ { '$eq': [ '$$value', None ] }, { '$gte': [ '$$val', '$$this.val' ] } ] }, { '$concat': [ { '$toString': { '$round': [ { '$divide': [ '$$val', '$$this.val' ] }, 2 ] } }, ' ', '$$this.key' ] }, '$$value' ] } } }, { '$round': [ '$$val', 1 ] } ] } }, 'in': { '$cond': [ '$$ispercent', { '$concat': [ { '$toString': { '$round': [ { '$multiply': [ '$$val', 100 ] }, 1 ] } }, '%' ] }, { '$ifNull': [ '$$calculatedVal', { '$toString': { '$round': [ '$$val', 2 ] } } ] } ] } } } } }, 'else': '$$this.v' } } } ] ] } } } } } } } ] ] }
    #     return self.pp.set({self.datakey: { '$arrayToObject': { '$reduce': { 'input': { '$objectToArray': f'${self.datakey}' }, 'initialValue': [], 'in': { '$let': { 'vars': { 'val': { 'curr': '$$this.v.curr', 'pre': '$$this.v.pre', 'growth': '$$this.v.growth' }, 'ispercent': { '$in': [ '$$this.k', '$percentkeys' ] } }, 'in': { '$concatArrays': [ '$$value', { '$cond': [ {"$cond": [{"$eq": [key, None]}, False, { '$ne': [ '$$this.k', key ] }]}, [], [ { 'k': '$$this.k', 'v': { '$mergeObjects': [ '$$this.v', { '$arrayToObject': { '$reduce': { 'input': { '$objectToArray': '$$val' }, 'initialValue': [], 'in': { '$concatArrays': [ '$$value', [ { 'k': '$$this.k', 'v': { '$let': { 'vars': { 'x': '$$this.v', 'isgrowth': { '$eq': [ '$$this.k', 'growth' ] } }, 'in': { '$let': { 'vars': { 'calculatedVal': self.getCalculatedValue('x', True) }, 'in': { '$cond': [ '$$ispercent', { '$cond': [ '$$isgrowth', { '$toString': {"$round": ["$$x",2]} }, { '$toString': { '$concat': [ { '$toString': '$$calculatedVal' }, '%' ] } } ] }, { '$cond': [ '$$isgrowth', { '$toString': { '$concat': [ { '$toString': '$$x' }, '%' ] } }, { "$toString": "$$calculatedVal" } ] } ] } } } } } } ] ] } } } } ] } } ] ] } ] } } } } } } })
