...
from models.collections.adv_rules import AdKey, AdRule, AdRuleAssetTypeWithAdProduct, CreateAdRuleRequest, SimplifyAdRuleRequest, getAdRuleSummary
from models.enums import AdAssetType, AdProduct, CollectionType, Operator
...
class AdRuleUtility:
    ...
    def __init__(self, db:AsyncIOMotorDatabase, uid: str, marketplace: ObjectId) -> None:
        ...
        self.uid = uid
        self.db = DbManager(db.get_collection(CollectionType.ADV_RULES.value), uid=self.uid, marketplace=self.marketplace)
    ...
    async def getCriteriaGroups(self, assettype: AdAssetType, adproduct: AdProduct):
        groups = await self.ruleCriteriaGroupsDb.find({"assettype": assettype.value, "adproduct": adproduct.value}, projectionInc=["action","subactions"])
        detail = await MarketplaceHelper(self.uid).getCountryBidsByMarketplace(self.marketplace)
