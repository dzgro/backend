...
from models.collections.adv_rule_runs import AdRuleRun, AdRuleRunRequest, AdRuleRunStatus
...
class AdRuleRunUtility:
    ...
    def __init__(self, db:AsyncIOMotorDatabase, uid: str, marketplace: ObjectId) -> None:
        ...
        self.uid = uid
        self.db = DbManager(db.get_collection(CollectionType.ADV_RULE_RUNS.value), uid=self.uid, marketplace=self.marketplace)
    ...
    def runRule(self, req: AdRuleRunRequest):
        try:
            rule = AdRuleUtility(self.marketplace, self.uid).getRuleById(req.ruleid)
            ...
            id = self.db.insertOne(obj, timestampkey="startedat", withUidMarketplace=True)
    ...
    async def getRuleRunResults(self, runid: str, paginator: Paginator):
        ...
        from app.HelperModules.Collections.dzgro.adv.rule_runs.RuleExecutors.RuleExecutor import AdRuleExecutor
