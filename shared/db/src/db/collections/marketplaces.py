...
from models.enums import CollectionType, AmazonAccountType, CountryCode, MarketplaceId, MarketplaceStatus
from models.model import Paginator, SuccessResponse, AddMarketplace
from db import Exceptions
from models.collections.marketplaces import Marketplace, UserAccountsCount, MarketplaceNameId, Account, AdAccount, RenameAccountRequest
from db.collections.pipelines import GetAdAccounts, GetMarketplaces
from models.collections.country_details import CountryDetailsWithBids
...
class MarketplaceHelper:
    ...
    def __init__(self, db: AsyncIOMotorDatabase, uid:str):
        self.uid = uid
        self.marketplaceDB = DbManager(db.get_collection(CollectionType.MARKETPLACES.value), uid=self.uid)
        self.spapiDB = DbManager(db.get_collection(CollectionType.SPAPI_ACCOUNTS.value), uid=self.uid)
        self.advDb = DbManager(db.get_collection(CollectionType.ADVERTISING_ACCOUNTS.value), uid=self.uid)
